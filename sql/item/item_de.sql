/*************************************************************************************************
  Item-Master Snapshot + Purchasing Intelligence  (v3-DE-6, May 2025)
  – IPG Laser GmbH data-mart view  (subsidiary = DE010) –
      • 9-month usage, open-PO posture
      • Last vendor / qty / unit-cost / country / PO / order date
      • item_index = 'DE010' + item_no   (no spaces)
      • **Revision No_ and HTS columns removed**
*************************************************************************************************/
SET NOCOUNT ON;
...
SELECT
    ROW_NUMBER() OVER (ORDER BY i.[No_])                           AS row_index,
    i.[No_]                                                        AS item_no,
    i.[Description],
    ...
    i.[Common Item No_]
INTO #Items
FROM [dbo].[IPG Laser GmbH$Item] i
WHERE i.[No_] <> '';
...
INTO #Ledger9m
FROM [dbo].[IPG Laser GmbH$Item Ledger Entry] le
...
FROM (
        SELECT phl.[No_], phl.[Quantity], phl.[Unit Cost (LCY)],
               ...
               phl.[Document No_]
        FROM   [dbo].[IPG Laser GmbH$Purchase Line Archive] phl
...
JOIN (
        ...
        FROM   [dbo].[IPG Laser GmbH$Purchase Header Archive]
        WHERE  [Document Type] = 1

        UNION ALL

        SELECT [No_], [Order Date]
        FROM   [dbo].[IPG Laser GmbH$Purchase Header]
        WHERE  [Document Type] = 1
     ) h
...
SELECT
    ...
    CONCAT('DE010', base.item_no COLLATE Latin1_General_100_CI_AS) AS item_index
FROM  base
