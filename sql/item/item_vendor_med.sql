/*************************************************************************************************
  Vendor‑Item Snapshot – US020  (v1 · May 2025)
  • Subsidiary: IPG Medical Corporation   (code = US020)
  • order_date pulled directly from line tables (history via header once)
  • Lean TempDB indexes, duplicate‑safe #Fact index
  • Spend metrics (last / 1‑y / 2‑y / all) and STDEV < 0.01 ⇒ 0
*************************************************************************************************/
SET NOCOUNT ON;
...
WITH hist AS (   /* posted‑history PO lines */
    SELECT
        'HIST'                              AS src,
    ...
    JOIN   [dbo].[IPG Medical Corporation$Purchase History Header] hhh
...
open_lines AS (  /* open / released PO lines */
    SELECT
        'OPEN'                             AS src,
...
LEFT JOIN [dbo].[IPG Medical Corporation$Vendor] v ON l.vendor_no=v.[No_];
