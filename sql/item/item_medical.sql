/*************************************************************************************************
  Item‑Master Snapshot + Purchasing Intelligence  (v3‑MC, May 2025)
  – IPG Medical Corporation data mart (subsidiary = US020) –
     • 9‑month usage, open‑PO posture
     • Last vendor / qty / unit‑cost / country
     • Last order date • last PO number
     • item_index = ‘US020’ + item_no   (no spaces)
**************************************************************************************************/
SET NOCOUNT ON;
...
SELECT
    ...
    i.[HTS]
INTO #Items
FROM [dbo].[IPG Medical Corporation$Item] i
WHERE i.[No_] <> '';
...
INTO #Ledger9m
FROM [dbo].[IPG Medical Corporation$Item Ledger Entry] le
...
INTO #OpenPO
FROM [dbo].[IPG Medical Corporation$Purchase Line] pl
...
/* ============================================================ 4. PO‑line fact set (last purchase fields)  */
...
SELECT
    ...
    SUM(l.[Quantity] * COALESCE(NULLIF(l.[Qty_ per Unit of Measure],0),1))
                                                                   AS quantity,
    MAX(l.[Unit Cost (LCY)] / COALESCE(NULLIF(l.[Qty_ per Unit of Measure],0),1))
                                                                   AS unit_cost
INTO #POLines
FROM (
    SELECT phl.[No_], phl.[Quantity], phl.[Unit Cost (LCY)],
           ...
           phl.[Document No_]
    FROM [dbo].[IPG Medical Corporation$Purchase History Line] phl
...
LEFT JOIN [dbo].[IPG Medical Corporation$Vendor] v
...
SELECT
    ...
    CASE
         ...
         ELSE 'No'
    END                                         AS raw_mat_flag,
    /* item index (subsidiary + item_no, no spaces) */
    'US020' + base.item_no                      AS item_index
FROM base
