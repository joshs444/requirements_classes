/*************************************************************************************************
  Vendor‑Item Snapshot – US010  (v2 · May 2025)
  • One‑pass, lean‑index version aligned with DE010 logic
  • order_date pulled directly from line tables (history rows via header once)
  • Non‑unique clustered index on #Fact (avoids dup‑key errors)
  • Spend metrics + STDEV < 0.01 coerced to 0
*************************************************************************************************/
SET NOCOUNT ON;
...
WITH hist AS (         /* posted‑history PO lines */
    ...
    JOIN   [dbo].[IPG Photonics Corporation$Purchase History Header] hhh
...
LEFT JOIN [dbo].[IPG Photonics Corporation$Vendor] v ON l.vendor_no=v.[No_];
