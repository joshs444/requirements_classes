/*************************************************************************************************
  Procurement Analytics – FULL STAND-ALONE SCRIPT   (v8.3-US-R2, May 2025)
  • Same business logic as v8.3-US, but ALL output columns are lower-case with "_"
**************************************************************************************************/
...
SELECT
    ...
    l.[Manufacturer Part No_]             AS manufacturer_part_no,
    l.[Manufacturer Code]                 AS manufacturer_code,
    l.[Total]                             AS total,
...
/* ---------- OUTER APPLY helpers (unchanged) ---------------- */
OUTER APPLY (
    SELECT SUM(ld.quantity * ld.[Unit Cost]) / NULLIF(SUM(ld.quantity),0) AS avg_price
    FROM   #LineData  ld
    JOIN   #HeaderData hd ON ld.[Document No_] = hd.[Doc_No_]
    WHERE  ld.[Type] = 'Item'
      AND  ld.[No_]  = l.[No_]
      AND  hd.[Order Date]
           BETWEEN DATEADD(day,-@w1y,h.[Order Date]) AND h.[Order Date]-1
) AS b1y
...
OUTER APPLY (
    ...
    WHERE  ld.[Type] = 'Item'
      AND  ld.[No_]  = l.[No_]
      AND  hd.[Order Date]
           BETWEEN DATEADD(day,-@w2y,h.[Order Date]) AND h.[Order Date]-1
) AS b2y

OUTER APPLY (
    SELECT SUM(ld.quantity * ld.[Unit Cost]) / NULLIF(SUM(ld.quantity),0) AS avg_price_vendor
    FROM   #LineData  ld
    JOIN   #HeaderData hd ON ld.[Document No_] = hd.[Doc_No_]
    WHERE  ld.[Type] = 'Item'
      AND  ld.[No_]  = l.[No_]
...
OUTER APPLY (
    ...
    WHERE  ld3.[Type] = 'Item'
      AND  ld3.[No_]  = l.[No_]
      AND  hd3.[Order Date]
           BETWEEN DATEADD(day,-@hv_window,h.[Order Date]) AND h.[Order Date]-1
) AS hv
